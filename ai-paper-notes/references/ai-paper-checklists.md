...
系统分类应由实际机制和可检查证据支持，不以表面相似性代替关键流程的确认。

## Agent、Workflow 或 Tool Use 工作
...
评测环境中的结果只支持其实验契约内的结论；对真实部署的推断需要相应的运行证据。
...
- Intended Construct：来源声称衡量的能力或质量到底是什么；
...
- 数据集、Evaluation harness、依赖、模型端点和报告运行结果能否复现。

聚合分数不能单独构成完整的质量解释；需结合失败分类和诊断证据说明结果。
...
安全指标应与可用性、误拒绝和剩余风险联合解释，不用单一指标代替完整判断。
...
性能结论应受实验硬件、Workload 和运行条件约束，超出实验契约时需要新证据。
